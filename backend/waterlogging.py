...
def _init_waterlogging_resources() -> None:
    ...
    if rasterio is not None and DEM_PATH.exists():
        try:
...
def get_street_waterlogging_geojson(
    ...
    # DEM Elevation Query via AW3D30 Raster Sampling
    try:
        from flood_map import sample_dem_elevations
        elevations_m = sample_dem_elevations(mid_lats, mid_lons)
    except Exception:
