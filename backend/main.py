...
def route(payload: RouteRequest) -> RouteResponse:
    ...
    return RouteResponse(
        ...
        scenario=str(res.get("scenario") or payload.scenario or "NORMAL").upper(),
        timestep=str(res.get("timestep") or payload.timestep or "T+0").upper(),
...
@app.get(
    "/pumps",
    summary="Get Permanent Pumping Stations Dynamic Registry & Telemetry",
    tags=["GIS & Infrastructure"],
)
def pumps(
    scenario: str = Query("NORMAL", description="Rainfall scenario (NORMAL, MODERATE, HEAVY, EXTREME)"),
    timestep: str = Query("T+0", description="Forecast timestep (T+0, T+1, T+2, T+3)"),
    rainfall_1h: float | None = Query(None, description="Optional custom 1h rainfall (mm)"),
) -> Dict[str, Any]:
    return get_pumps_metadata(scenario=scenario, timestep=timestep, rainfall_1h=rainfall_1h)
